...
public class TestService extends Service {
    public final static String TAG = TestService.class.getSimpleName();
    int ix=1;
    ...
    private Handler handler = new Handler(){
        ...
        public void handleMessage(Message msg) {
            switch (msg.what){
                ...
                case RESTART:
                    Intent dialogIntent = new Intent(TestService.this, MainActivity.class);
                    dialogIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK); //必须添加 Intent.FLAG_ACTIVITY_NEW_TASK
                    TestService.this.startActivity(dialogIntent);
...
//        if (mBuilder == null) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                NotificationChannel channel = new NotificationChannel("10", "ForeGround", NotificationManager.IMPORTANCE_DEFAULT);
//                mBuilder.setChannelId("10");
                notificationManager.createNotificationChannel(channel);
            }
    ...
    private class CoverServiceConnection implements ServiceConnection {
        ...
        public void onServiceConnected(ComponentName name, IBinder binder) {
            ...
            Service helpService = ((HelpService.LocalBinder) binder)
                    .getService();
            TestService.this.startForeground(PID, getNotification());
            helpService.startForeground(PID, getNotification());
            helpService.stopForeground(STOP_FOREGROUND_DETACH);

            TestService.this.unbindService(mConnection);
            mConnection = null;
        }
    }
